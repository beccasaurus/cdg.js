...
function CDG() {
  ...
  this._read = function() {
    ...
    if ((this._readPackets * packetSizeInBytes) < this._arrayBuffer.byteLength) {
      ...
      console.log(["Readying 24 bytes starting at", startByte]);
      ...
      var cdg = this;
      setTimeout(function() { cdg._read.call(cdg); }, 1);
    }
  };
  ...
  var packetFromBytes = function(packetBytes) {
    ...
    console.log("Data bytes for " + packet.instruction);
  ...
  var onTileBlockNormal = function(dataBytes, packet) {
    ...
    var columnIndex = dataBytes[2] & 0x1F
    var rowIndex = dataBytes[3] & 0x3F;
    console.log(["NORMAL column/row + color1/color2", columnIndex, rowIndex, color0, color1]);
    // var columnIndex = (dataBytes[2] & 0x1F) * 12;
    // var rowIndex = (dataBytes[3] & 0x3F) * 6;
    
    // row = TILE_HEIGHT * (rowIndex+1)
    // column = TILE_WIDTH * (columnIndex+1)
    // indexOfAllPixels = row * FULL_WIDTH + column
    // int[] of [FULL_WIDTH * FULL_HEIGHT]
    ...
    for (var i = 0; i < 12; i++) {
      ...
      for (var j = 0; j < 6; j++) {
        ...
        this.colorIndexesByPosition[position] = colorIndex;
      }
    }
  }

  var onTileBlockXOR = function(dataBytes, packet) {
    var color0 = dataBytes[0] & masks.lower4bits;
    var color1 = dataBytes[1] & masks.lower4bits;
    var columnIndex = dataBytes[2] & 0x1F
    var rowIndex = dataBytes[3] & 0x3F;
    console.log(["XOR column/row + color1/color2", columnIndex, rowIndex, color0, color1]);
    ...
    for (var i = 0; i < 12; i++) {
      ...
      for (var j = 0; j < 6; j++) {
        ...
        var pixelChoice = (byte >> (5 - j)) & 0x01;
        var colorIndex = pixelChoice == 0 ? color0 : color1;
        var colorIndexToUse = this.colorIndexesByPosition[position] ^ colorIndex;
        packet.pixels.push({ position: position, color: this.colorTable[colorIndexToUse] });
        this.colorIndexesByPosition[position] = colorIndexToUse;
      }
    }
  }
